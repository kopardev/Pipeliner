...
class ATACSeqFrame( PipelineFrame ) :
    def __init__(self, pipepanel, pipeline_name, *args, **kwargs) :
        ...
        PipelineLabels=["Peak Calling", "Differential Peak Analysis" ]
        Pipelines=["atacseq","diffatac"]
...
#         #rMinsamples.trace('w', lambda a,b,c,x="rmincount": makejson(x))
    ...
    def option_controller( self, *args, **kwargs ) :
        ...
        if self.Pipeline.get() == 'atacseq' :
            self.om_groups.grid(row=10,column=0, columnspan=6, sticky=W, padx=20, pady=10)
            self.om_coming_soon.grid_forget()
        elif self.Pipeline.get() == 'diffatac' :
            self.om_coming_soon.grid(row=10,column=0, columnspan=6, sticky=W, padx=20, pady=10)
